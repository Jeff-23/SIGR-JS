...
export class UsuariosService {
  ...
  async update(
    ...
    const usuarioObjetivo =
      await this.buscarUsuarioDentroDelAlcance(
        id,
        usuarioActual,
      );
    ...
    let restauranteDestino =
      usuarioObjetivo.restauranteId;

    let sucursalDestino =
      usuarioObjetivo.sucursalId;

    // SUPERADMIN SIGR.
    if (usuarioActual.restauranteId === null) {
      if (
        updateUsuarioDto.restauranteId !==
        undefined
      ) {
        restauranteDestino =
          updateUsuarioDto.restauranteId;
    ...
    if (
      updateUsuarioDto.password !== undefined
    ) {
      passwordHasheada =
        await bcrypt.hash(
