...
export function PublicQrMenuPage() {
  ...
  useEffect(() => {
    if (token.startsWith("demo-mesa-")) {
      const timer = window.setTimeout(() => setMenu({ restaurante: "Restaurante El Mono", sucursal: "La Carolina", mesa: { numero: token.replace("demo-mesa-", "") }, modoQr: "PEDIDO_CON_APROBACION", pedidosHabilitados: true, requiereAceptacion: true, plantillaCarta: { titulo: "Menú de almuerzos", subtitulo: "Cocina fresca todos los días", estilo: "EDITORIAL_DORADO", mostrarPrecios: true, fondoColor: "#F3EDE1", tarjetaColor: "#FFFDF8", textoColor: "#14283B", acentoColor: "#B98A2D", encabezadoColor: "#14283B", mostrarImagenesProductos: true }, cartaDia: { fecha: "", contenido: { especial: { titulo: "Especial de hoy", nombre: "Carne desmechada", descripcion: "En salsa criolla" } } }, categorias: demoCategories }), 0);
  ...
  const cardOpacity = activeTemplate?.tarjetaOpacidad ?? 0.82;
  const vars = { "--qr-bg": palette.bg, "--qr-card": palette.card, "--qr-card-alpha": hexToRgba(palette.card, cardOpacity), "--qr-dark": palette.dark, "--qr-accent": palette.accent, "--qr-text": palette.text, "--qr-muted": palette.muted } as CSSProperties;
  ...
  return <main style={vars} className={`min-h-screen bg-[var(--qr-bg)] text-[var(--qr-text)] ${menu.pedidosHabilitados ? "pb-32" : "pb-10"}`}>
    ...
    <div className="mx-auto max-w-4xl space-y-8 p-5 sm:p-7">
      {(menu.perfilesCarta?.length ?? 0) > 1 && <section className="rounded-2xl bg-[var(--qr-card-alpha)] p-3 shadow-sm"><p className="px-2 pb-2 text-xs font-black uppercase tracking-[.16em] text-[var(--qr-muted)]">Elige la carta</p><div className="flex flex-wrap gap-2">{menu.perfilesCarta?.map((item) => <button key={item.id} onClick={() => setSelectedProfileId(item.id)} className={`rounded-full px-4 py-2 text-sm font-black ${activeProfile?.id === item.id ? "bg-[var(--qr-dark)] text-white" : "border border-black/10"}`}>{item.nombre}</button>)}</div></section>}
      ...
      {menu.pedidosHabilitados && <section className="rounded-[1.75rem] bg-[var(--qr-card-alpha)] p-5 shadow-sm"><h2 className="font-black">Datos del pedido</h2><input className="mt-4 w-full rounded-2xl border border-black/10 bg-transparent p-3" placeholder="Nombre (opcional)" value={name} onChange={(event) => setName(event.target.value)}/><textarea className="mt-3 w-full rounded-2xl border border-black/10 bg-transparent p-3" placeholder="Observaciones generales (opcional)" value={notes} onChange={(event) => setNotes(event.target.value)}/><p className="mt-3 text-xs text-[var(--qr-muted)]">{menu.requiereAceptacion ? "El restaurante confirmará el pedido antes de ingresarlo a operación." : "El pedido ingresará automáticamente a operación."}</p>{failure && <p className="mt-3 text-sm font-bold text-red-700">{failure}</p>}</section>}
    </div>
