...
export function RealKitchenPage() {
  ...
  const [newStation, setNewStation] = useState({ codigo: "", nombre: "", color: "#8B5CF6", objetivoPreparacionMin: 15 });
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (newcomers.length && known.current.size > 0) {
        toast.success(`${newcomers.length} nueva(s) comanda(s)`, { icon: "🔔", duration: 6000 });
        if (soundEnabled.current) beep(880);
      }
      if (newlyReady.length && ready.current.size > 0) {
        toast.success(`${newlyReady.length} comanda(s) lista(s) para servicio`, { icon: "✅" });
  ...
  const updateLineState = (command: Command, line: CommandLine, estado: "EN_PREPARACION" | "LISTA") => act(
    `line-${line.id}`,
    () => api.patch(`/comandas/${command.id}/detalles/${line.id}/estado`, { estado }),
  );

  const prioritize = (command: Command, prioridad: KdsPriority) => act(
    `priority-${command.id}`,
  ...
  const createStation = async () => {
    ...
    try {
      ...
      setNewStation({ codigo: "", nombre: "", color: "#8B5CF6", objetivoPreparacionMin: 15 });
      await load(true);
    } catch (error) {
      toast.error(errorMessage(error));
    }
  ...
  return <div>
    ...
    {stationForm && <div className="card mt-4 grid gap-3 md:grid-cols-[1fr_1.4fr_110px_130px_auto]">
      <input className="input h-11" maxLength={40} placeholder="Código: POSTRES" value={newStation.codigo} onChange={(event) => setNewStation({ ...newStation, codigo: event.target.value })}/>
      <input className="input h-11" maxLength={80} placeholder="Nombre de estación" value={newStation.nombre} onChange={(event) => setNewStation({ ...newStation, nombre: event.target.value })}/>
      <input aria-label="Color de estación" className="h-11 w-full rounded-xl bg-white p-1" type="color" value={newStation.color} onChange={(event) => setNewStation({ ...newStation, color: event.target.value })}/>
      <label className="text-xs font-black text-denim/55">Meta (min)<input className="input mt-1 h-8 px-2" min={1} max={240} type="number" value={newStation.objetivoPreparacionMin} onChange={(event) => setNewStation({ ...newStation, objetivoPreparacionMin: Number(event.target.value) || 15 })}/></label>
      <button className="primary h-11 px-5" onClick={() => void createStation()}>Crear</button>
    </div>}
...
function CommandCard({
  ...
  updateCommandState: (command: Command, estado: "LISTA" | "ENTREGADA") => Promise<void>;
  updateLineState: (command: Command, line: CommandLine, estado: "EN_PREPARACION" | "LISTA") => Promise<void>;
  ...
  const pendingLines = command.detalles.some((line) => line.estado === "PENDIENTE");

  return <article className={["kds-card", level, !command.fechaVista ? "unseen" : ""].join(" ")}>
    {!command.fechaVista && <div className="kds-unseen-ribbon"><BellRing size={14}/> NUEVO · REQUIERE VISTO</div>}
    <header className="kds-header" style={{ borderTopColor: command.estacion.color }}>
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-denim/45">{command.estacion.codigo === "BAR" ? <Martini size={16}/> : <ChefHat size={16}/>} {command.estacion.nombre}</div>
        <h2 className="mt-1 truncate text-3xl font-black">{commandDestination(command)}</h2>
        <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs font-bold text-denim/45">
          ...
          {command.pedido.mesero && <span>Mesero: {command.pedido.mesero.nombres}</span>}
        </div>
      </div>
    ...
    <div className="kds-lines flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
      <div className="space-y-3">
        {command.detalles.map((line) => <LineCard
          key={line.id}
          line={line}
          command={command}
    ...
    <footer className="border-t border-denim/8 bg-white p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <span className={["status-pill", command.estado === "LISTA" ? "bg-emerald-100 text-emerald-800" : ""].join(" ")}>{command.estado.replaceAll("_", " ")}</span>
...
function PromisePanel({ promise }: { promise: ReturnType<typeof servicePromise> }) {
  if (promise.risk === "late") {
    return <div className="kds-promise late"><div><strong><AlertTriangle size={18}/> Fuera de objetivo</strong><span>Meta {promise.target} min</span></div><b>+{Math.abs(promise.remaining)} min</b></div>;
  }
  return <div className={`kds-promise ${promise.risk}`}>
    <div><span>Meta</span><strong>{promise.target} min</strong></div>
    <div><span>Lleva</span><strong>{promise.elapsed} min</strong></div>
    <div><span>Quedan</span><strong>{promise.remaining} min</strong></div>
    <div><span>Riesgo</span><strong>{riskLabel[promise.risk]}</strong></div>
  </div>;
}

function LineCard({ line, command, busy, canEdit, updateLineState }: {
  line: CommandLine;
  command: Command;
  busy: boolean;
  canEdit: boolean;
  ...
  return <div className={["kds-line", line.estado.toLowerCase()].join(" ")}>
    <div className="flex items-start gap-3">
      <b className="kds-qty">{line.cantidad}×</b>
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <strong className="text-lg leading-tight">{line.detallePedido.producto.nombre}</strong>
    ...
    {canEdit && line.estado !== "LISTA" && <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
      {line.estado === "PENDIENTE" && <button disabled={busy} className="kds-line-action" onClick={() => void updateLineState(command, line, "EN_PREPARACION")}><Flame size={17}/> Iniciar línea</button>}
      <button disabled={busy} className="kds-line-action ready" onClick={() => void updateLineState(command, line, "LISTA")}><CheckCircle2 size={17}/> Línea lista</button>
    </div>}
    {line.estado === "LISTA" && <div className="mt-3 flex items-center gap-2 text-xs font-black text-emerald-700"><CheckCircle2 size={16}/> Preparación terminada</div>}
  </div>;
}
