...
export function RealSalonPage() {
  ...
  const load = useCallback(
    async (quiet = false) => {
      ...
      try {
        ...
        const [tableResponse, productResponse, orderResponse] =
          await Promise.all([
  ...
  const visibleProducts = products.filter(
    (product) =>
      (category === "all" || product.categoria.id === category) &&
      product.nombre.toLowerCase().includes(search.toLowerCase()),
  );
  const orderByTable = new Map(
    orders
      .filter((order) => order.mesa)
      .map((order) => [order.mesa!.id, order]),
  );
  ...
  const add = (product: ApiProduct) =>
    setCart((current) => {
      ...
      return found
        ...
        : [...current, { product, quantity: 1, notes: "" }];
    });
  ...
  const setNotes = (id: number, notes: string) =>
    setCart((current) =>
      current.map((line) =>
        line.product.id === id ? { ...line, notes } : line,
      ),
    );
  ...
  const submit = async () => {
    ...
    try {
      if (draft.existing) {
        ...
        const result = await api.post<ApiOrder>(
          `/pedidos/${draft.existing.id}/detalles`,
          { detalles: details },
        );
        const updated = result.data;
        setCart([]);
        setDraft({ ...draft, existing: updated });
        await sendPending({
          ...updated,
        ...
        if ("queued" in result)
          toast.success(
            "Pedido guardado sin conexión; quedará pendiente de envío a preparación",
          );
        else {
          const created = result.data as ApiOrder;
          setCart([]);
          setDraft({ ...draft, existing: created });
          await sendPending(created);
  ...
  const occupy = async (table: ApiTable) => {
    try {
      await api.patch(`/mesas/${table.id}/ocupar-sin-pedido`, {
        motivo: "Cliente ubicado sin pedido",
      });
      toast.success(`Mesa ${table.numero} ocupada`);
      await load(true);
    } catch (error) {
      toast.error(errorMessage(error));
    }
  ...
  const release = async (table: ApiTable) => {
    try {
      await api.patch(`/mesas/${table.id}/liberar-sin-consumo`, {
        motivo: "Cliente se retiró sin consumo",
      });
      toast.success(`Mesa ${table.numero} liberada`);
      await load(true);
    } catch (error) {
      toast.error(errorMessage(error));
    }
  ...
  const cancelOrder = async (order: ApiOrder) => {
    if (
      !window.confirm(
        `¿Cancelar el pedido #${order.id}? Esta acción quedará auditada.`,
      )
    )
      return;
    try {
    ...
    <div>
      <div className="section-title">
        <div>
          ...
          <h1 className="page-title">Salón y pedidos</h1>
        </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
        {visibleTables.map((table) => {
          ...
          return (
            <article
              className={`table-card ${table.situacion.toLowerCase()}`}
              key={table.id}
            >
              <span className="text-xs font-bold uppercase tracking-wider opacity-45">
                {table.zona.nombre}
              </span>
              <strong className="mt-2 text-3xl font-black">
                Mesa {table.numero}
              </strong>
              <span className="mt-2 flex items-center gap-1 text-xs opacity-55">
                <Users size={13} />
                {table.capacidad} personas
              </span>
              <span className="mt-4 text-xs font-extrabold">
                {table.situacion === "LIBRE"
                  ? "Disponible"
                  : table.situacion === "OCUPADA"
                    ? "En servicio"
                    : "Pendiente de pago"}
              </span>
              {table.situacion === "LIBRE" && (
                <>
                  <button
                    className="primary mt-3 h-10 text-xs"
                  ...
                  {hasPermission("MESAS_EDITAR") && (
                    <button
                      className="mt-2 text-xs font-bold text-denim/55"
              ...
              {order && (
                <button
                  className="primary mt-3 h-10 text-xs"
                  onClick={() => void openExisting(order)}
                >
                  Ver pedido #{order.id}
                </button>
              )}
      ...
      <section className="mt-8">
        <div className="section-title">
          <div>
            ...
            <h2 className="text-2xl font-black">Pedidos activos</h2>
          </div>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-3">
          {orders.map((order) => (
            <button
              className="card flex min-h-36 flex-col text-left"
              key={order.id}
              onClick={() => void openExisting(order)}
            >
              ...
              <span className="mt-3 text-sm text-denim/55">
                {order.detalles.reduce(
                  (sum, detail) => sum + detail.cantidad,
                  0,
                )}{" "}
                productos · {money.format(Number(order.total))}
              </span>
              <span className="mt-auto pt-4 text-xs font-bold text-denim/45">
                Abrir cuenta preliminar y gestionar
              </span>
            </button>
          ))}
        </div>
      ...
      {draft && (
        <div className="fixed inset-0 z-50 flex justify-end bg-steel/45">
          <section className="h-full w-full max-w-3xl overflow-y-auto bg-[#f7f5ef] p-5 sm:p-8">
            <div className="section-title">
              <div>
                <p className="eyebrow">
                  {draft.existing
                    ? `Pedido #${draft.existing.id}`
                    : "Nuevo pedido"}
                </p>
            ...
            {draft.existing && (
              <div className="mt-5 card">
                <div className="flex items-center gap-2">
                  <ClipboardList />
                  <h3 className="font-black">Cuenta preliminar</h3>
                </div>
                {draft.existing.detalles.map((detail) => (
                  ...
                  >
                    <span>
                      {detail.cantidad}× {detail.producto.nombre}
                    ...
                    <b>{money.format(Number(detail.subtotal))}</b>
                  </div>
                ))}
                <div className="mt-4 flex justify-between border-t border-denim/10 pt-4">
                  <b>Total actual</b>
                ...
                {pendingCommandDetails(draft.existing).length > 0 &&
                  ...
                  hasCapability("KDS") && (
                    <button
                      className="secondary mt-4 h-11"
                      onClick={async () => {
                        try {
                          await sendPending(draft.existing!);
                          toast.success("Pendientes enviados a preparación");
                          await openExisting(draft.existing!);
                        } catch (error) {
                          toast.error(errorMessage(error));
                        }
                      }}
                    >
                      <AlertCircle size={16} />
                      Enviar pendientes a preparación
                    </button>
                  )}
              </div>
            )}
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {visibleProducts.map((product) => (
                <button
                  ...
                  key={product.id}
                >
                  <span className="h-10 w-1 rounded-full bg-orange-400" />
                  <span className="flex-1">
                    <strong className="block">{product.nombre}</strong>
            ...
            <div className="mt-6 card">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-black">
                  {draft.existing ? "Agregar productos" : "Pedido"}
                </h3>
              </div>
              {!cart.length ? (
                ...
                cart.map((line) => (
                  <div
                    className="border-b border-denim/7 py-3"
                    key={line.product.id}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <b className="w-24 text-right">
                        {money.format(
                          Number(line.product.precio) * line.quantity,
                        )}
                      </b>
                    </div>
                    <input
                      className="mt-2 w-full rounded-xl border border-denim/10 bg-denim/[.02] px-3 py-2 text-sm outline-none focus:border-marigold"
                      maxLength={300}
                      placeholder="Observaciones: sin cebolla, término medio…"
                      value={line.notes}
                      onChange={(e) =>
                        setNotes(line.product.id, e.target.value)
                      }
                    />
                  </div>
                ))
              ...
              <div className="mt-5 flex justify-between">
                <span className="text-sm text-denim/45">
                  {draft.existing ? "Adición" : "Total estimado"}
                </span>
                <strong className="text-2xl">
                  {money.format(
                    cartTotal(cart) +
                      (!draft.existing && draft.type === "DOMICILIO"
                        ? delivery.costo
                        : 0),
                  )}
                </strong>
              ...
              >
                {saving
                  ? "Guardando…"
                  : draft.existing
                    ? "Agregar y enviar a preparación"
                    : "Crear y enviar a preparación"}
              </button>
              {draft.existing && hasPermission("PEDIDOS_CANCELAR") && (
                <button
                  className="mt-4 w-full text-sm font-bold text-red-600"
